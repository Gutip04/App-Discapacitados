...
export default function GradoEstudio() {
  const [data, setData] = useState<GradoEstudio[]>([]);
  const [totalGeneral, setTotalGeneral] = useState(0);

  useEffect(() => {
    async function fetchData() {
      try {
        const resultado = await EstadisticasService.GradoEstudio();
        const datos: GradoEstudio[] = resultado.data;

        const total = datos.reduce((acc, curr) => acc + curr.total, 0);
        setTotalGeneral(total);
        setData(datos);
      } catch (error) {
        console.error("Error al obtener estadísticas de grado de estudio:", error);
      }
    }
    fetchData();
  }, []);
