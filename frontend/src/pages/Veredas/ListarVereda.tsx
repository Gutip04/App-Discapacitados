...
export default function ListarVereda(){
    const {
        eliminarVereda,
        error,
        filtros,
        ...
        total,
        veredas 
    } = UseVeredas({nombre:"", page: 1, limit:5})

    const [mensaje, setMensaje] = useState("");
    const [tipoMensaje, setTipoMensaje] = useState<"success" | "error" | "">("");
    const [veredaAEliminar, setVeredaAEliminar] = useState<Vereda | null>(null);
    ...
    const confirmarEliminacion = async () =>{
        ...
        if(!veredaAEliminar) return

        try{
            await eliminarVereda(veredaAEliminar?.id)
            setMensaje(`Vereda ${veredaAEliminar.nombre} Eliminado Correctamente`)
            setTipoMensaje("success")
        } catch {
            setMensaje(`No se pudo eliminar el barrio ${veredaAEliminar.nombre}.`)
            setTipoMensaje("error")
        } finally {
            setVeredaAEliminar(null)
            setTimeout(() => {
                setMensaje("");
                setTipoMensaje("");
            }, 5000);
  ...
  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
    <h1 className="text-3xl font-extrabold uppercase text-gray-800">Veredas</h1>
  ...
  {loading && <p className="text-gray-500">Cargando veredas...</p>}
      ...
      <tbody>
        {veredas.map((v, index) => (
          ...
          >
            ...
            <td className="py-3 px-4 font-medium text-gray-700">{v.nombre}</td>
            <td className="py-3 px-4 text-center flex justify-center gap-2">
              <BotonFormulario
                to={`/veredas/editar/${v.id}`}
                texto="Editar"
                color="blue"
                py={2}
                px={4}
...
<div className="flex flex-col justify-center items-center gap-4 mt-6">
  ...
  <ConfirmModal
    visible={!!veredaAEliminar}
    title="¿Eliminar Vereda?"
    message={`¿Estás seguro de que quieres eliminar "${veredaAEliminar?.nombre}"?`}
    onCancel={() => setVeredaAEliminar(null)}
