...
export default function ListarEps() {
    const {
        ...
        eliminarEps,
        setFiltros
    } = UseEps({ nombre: "", page: 1, limit: 5  });
    
    const [mensaje, setMensaje] = useState("");
    const [tipoMensaje, setTipoMensaje] = useState<"success" | "error" | "">("");
    const [epsAEliminar, setEpsAEliminar] = useState<Eps | null>(null);
    ...
    const confirmarEliminacion = async () => {
        ...
        if (!epsAEliminar) return;

    try {
        await eliminarEps(epsAEliminar.id);
        setMensaje(`Barrio ${epsAEliminar.nombre} eliminado correctamente.`);
        setTipoMensaje("success");
    } catch {
        setMensaje(`No se pudo eliminar el barrio "${epsAEliminar.nombre}".`);
        setTipoMensaje("error");
    } finally {
        setEpsAEliminar(null);
        setTimeout(() => {
            setMensaje("");
            setTipoMensaje("");
        }, 5000);
  ...
  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
    <h1 className="text-3xl font-extrabold uppercase text-gray-800">Eps</h1>

    <BotonFormulario to="/eps/crear" texto="Agregar" color="green" px={4} py={2} />
  </div>
  ...
  {loading && <p className="text-gray-500">Cargando Eps...</p>}
      ...
      <tbody>
        {eps.map((e, index) => (
          ...
          >
            ...
            <td className="py-3 px-4 font-medium text-gray-700">{e.nombre}</td>
...
<div className="flex flex-col justify-center items-center gap-4 mt-6">
  ...
  <ConfirmModal
    visible={!!epsAEliminar}
    title="¿Eliminar eps?"
    message={`¿Estás seguro de que quieres eliminar "${epsAEliminar?.nombre}"?`}
    onCancel={() => setEpsAEliminar(null)}
